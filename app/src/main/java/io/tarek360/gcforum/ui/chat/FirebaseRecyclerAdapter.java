...
public abstract class FirebaseRecyclerAdapter<T, VH extends RecyclerView.ViewHolder>
   ...
   * that you provide
   * @param modelLayout This is the layout used to represent a single item in the list. You will be
  ...
  public FirebaseRecyclerAdapter(Class<T> modelClass, int modelLayout, Class<VH> viewHolderClass,
      Query ref, RecyclerView recyclerView) {
      ...
      @Override public void onChanged(EventType type, int index, int oldIndex) {
        switch (type) {
          case Added:
            ...
            Logger.debug("XOXOX",
                "mSnapshots.getCount()" + mSnapshots.getCount() + " index " + index);
            if (index == mSnapshots.getCount()) {
  ...
  @Override public VH onCreateViewHolder(ViewGroup parent, int viewType) {
    ViewGroup view =
        (ViewGroup) LayoutInflater.from(parent.getContext()).inflate(mModelLayout, parent, false);
    try {
  ...
  /**
   * Each time the data at the given Firebase location changes, this method will be called for each
   * item that needs
   * to be displayed. The first two arguments correspond to the mLayout and mModelClass given to
   * the
   * constructor of
