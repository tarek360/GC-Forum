...
import io.tarek360.gcforum.ui.customViews.SpacesItemDecoration;
...
public class SponsorsFragment extends BaseFragment implements SponsorsContract.View {
   ...
   * number.
   */
  public static SponsorsFragment newInstance(String title) {
  ...
  @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    mActionsListener = new SponsorsPresenter(this);

    initRV();

    mActionsListener.loadSponsors();

    swipeContainer.setOnRefreshListener(mActionsListener::loadSponsors);
  ...
  private void initRV() {
    ...
    int spacingInPixels = getResources().getDimensionPixelSize(R.dimen.sponsor_item_space);
    rvProducts.addItemDecoration(new SpacesItemDecoration(spacingInPixels));

    mSponsorsSections = new ArrayList<>();
    // Create adapter passing in the sample user data
    mRVAdapter = new RVAdapter(mSponsorsSections, mActionsListener, mOptions);
    GridLayoutManager layoutManager =
        new GridLayoutManager(getActivity(), getResources().getInteger(R.integer.grid_span));
    rvProducts.setLayoutManager(layoutManager);
    mRVAdapter.setLayoutManager(layoutManager);
  ...
  @Override public void showSponsors(List<SponsorsSection> sponsorsSections) {
    mRVAdapter.setProducts(sponsorsSections);
  }
  ...
  @Override public void openSponsorUrl(String url) {
    ...
    if (TextUtils.isEmpty(url)) {
      Logger.error(this, "No url");
    } else if (!Utils.openUrl(getActivity(), url)) {
  ...
  private void initUILOptions() {
    ...
    Drawable drawable =
        new IconDrawable(getActivity(), MaterialCommunityIcons.mdi_briefcase_check).colorRes(
            R.color.md_grey_100);
